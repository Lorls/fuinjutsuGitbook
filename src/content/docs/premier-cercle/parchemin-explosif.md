---
title: Parchemin explosif
description: Talisman du 1er cercle qui stocke du chakra explosif, détoné à distance ou au contact.
sidebar:
  order: 5
---

<figure class="seal-icon">
<svg width="64" height="64" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#3b82f6" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
  <circle cx="24" cy="24" r="21" opacity="0.3"/>
  <path d="M24 11 l2.5 6 l6 -2.5 l-2.5 6 l6 2.5 l-6 2.5 l2.5 6 l-6 -2.5 l-2.5 6 l-2.5 -6 l-6 2.5 l2.5 -6 l-6 -2.5 l6 -2.5 l-2.5 -6 l6 2.5 z"/>
  <circle cx="24" cy="24" r="3" fill="#3b82f6" stroke="none"/>
</svg>
</figure>

| Cercle | Kanjis | Roll |
|---|---|---|
| 1er cercle | 起 爆 (起爆) | > 50 |

## Définition

Talisman qui **stocke du chakra explosif** dans un parchemin. Il **détone** à distance (via un fil de chakra) ou au contact, projetant une déflagration.

## Kanjis

Deux kanjis formant le mot **起爆** (kibaku, « détonation ») :

- **起** — déclencher / amorcer
- **爆** — explosion

## Règles

- **Consommable** : le parchemin est détruit après usage.
- La **puissance et le rayon** de l'explosion dépendent du niveau du lanceur.
- Détonation **à distance** (fil de chakra), **au contact** ou par **minuterie**.
- Ne peut **pas être posé directement sur un être vivant**.
- Préciser l'usage dans le `/it` ; preuve recommandée.
