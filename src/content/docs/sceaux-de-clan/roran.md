---
title: Sceau des Roran
description: Sceau de Juinjutsu liant deux jumeaux à vie après la réussite d'un rituel.
sidebar:
  order: 2
---

<figure class="seal-icon">
<svg width="64" height="64" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#f59e0b" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
  <circle cx="24" cy="24" r="21" opacity="0.3"/>
  <circle cx="19" cy="24" r="6"/>
  <circle cx="29" cy="24" r="6"/>
  <path d="M24 20 v8" opacity="0.6"/>
</svg>
</figure>

| Type | Clan |
|---|---|
| Juinjutsu | Roran — exclusif |

## Définition

Sceau de **Juinjutsu** liant **2 jumeaux** après la réussite d'une **épreuve (rituel)**. Ceux-ci sont alors **liés à vie** : ce sceau lie leurs **sentiments et leurs âmes** jusqu'à leur mort.

## Règles

- Seul le **chef de clan** peut le poser **si le sceau lui a été enseigné** ; sinon, il devra le **découvrir d'une autre manière**.
- Un Roran s'intéressant au Fuin aura **potentiellement la capacité de comprendre ce sceau et de le reproduire**.
